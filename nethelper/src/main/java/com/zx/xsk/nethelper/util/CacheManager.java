package com.zx.xsk.nethelper.util;
...
import com.zx.xsk.nethelper.inters.CacheCallback;
...
public final class CacheManager {
    ...
    private CacheManager() {
        File diskCacheDir = getDiskCacheDir(IApplication.application.getApplicationContext(), CACHE_DIR);
        ...
        if (diskCacheDir.getUsableSpace() > DISK_CACHE_SIZE) {
            try {
                mDiskLruCache = DiskLruCache.open(diskCacheDir,
                        getAppVersion(IApplication.application.getApplicationContext()), 1/*一个key对应多少个文件*/, DISK_CACHE_SIZE);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
